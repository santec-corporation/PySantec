...
def configure_mpm(mpm: MPMInstrument,
    ...
    mpm.set_range_mode(mpm_enums.RangeMode.MANUAL)
    mpm.set_range_value(1)      # Sets the first dynamic range value (-30 ~ +10 dBm)
    ...
    mpm.set_scan_parameters(start_wavelength,
                            ...
                            scan_speed,
                            tsl_actual_step,
                            mpm_enums.MeasurementMode.SWEEP1)
    while True:
        if mpm.get_measurement_mode() == mpm_enums.MeasurementMode.SWEEP1:
            break
        mpm.set_measurement_mode(mpm_enums.MeasurementMode.SWEEP1)
